...
uint8_t crc8_lut_volkswagen_lwi_01[] = {
...
unsigned int volkswagen_checksum(unsigned int address, uint64_t d, int length)
{
  ...
  // payload content plus a single byte of pseudo-secret padding.
  ...
  switch(address) {
    case 0x86:
      ...
      break;
    case 0x9f:
      crc ^= crc8_lut_volkswagen_eps_01[counter];
      break;
    case 0xad:
      crc ^= crc8_lut_volkswagen_getriebe_11[counter];
      break;
    case 0x106:
      crc ^= crc8_lut_volkswagen_esp_05[counter];
      break;
    case 0x126:
      crc ^= crc8_lut_volkswagen_hca_01[counter];
      break;
    case 0x12b:
      crc ^= crc8_lut_volkswagen_gra_acc_01[counter];
      break;
    case 0x3c0:
      crc ^= crc8_lut_volkswagen_klemmen_status_01[counter];
      break;
    case 0x65d:
      crc ^= crc8_lut_volkswagen_esp_20[counter];
      break;
    default:
      INFO("Attempt to CRC check undefined Volkswagen message 0x%02X\n", address);
      crc ^= crc8_lut_volkswagen_undefined[counter];
      break;
  }
  crc = crc8_lut_8h2f[crc];
