...
{
  // The Volkswagen CRC algorithm is standard CRC8 8H2F/AUTOSAR, with a magic
  // padding byte tacked onto the end of the payload.
  // https://www.autosar.org/fileadmin/user_upload/standards/classic/4-3/AUTOSAR_SWS_CRCLibrary.pdf

  uint8_t crc = 0xFF; // Initial CRC value for CRC8 8H2F/AUTOSAR

  // The algorithm is first run over the 4 to 8 byte payload, skipping over the
  // first byte which is reserved for the CRC itself.
  // FIXME: There is some endianness issue here that I'm not sure how to fix
  // correctly; for now we iterate backwards through the payload.
  ...
  // Look up the magic value for the CRC padding byte, which permutes by CAN
  // address, and additionally (for SOME addresses) by the message counter.
  uint8_t counter = dat[6] & 0x0F;
  switch(address) {
    ...
    case 0xFD:  // ESP_21 Electronic Stability Program
      crc ^= (uint8_t[]){0x1F,0xEF,0xF8,0x49,0x1E,0xE5,0xC2,0xC0,0x97,0x19,0x3C,0xC9,0xF1,0x98,0xD6,0x61}[counter];
      break;
  ...
  return crc ^ 0xFF; // Return after final XOR for CRC8 8H2F/AUTOSAR
}
