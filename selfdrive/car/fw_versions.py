...
UDS_VM_SPARE_PART_REQUEST = bytes([uds.SERVICE_TYPE.READ_DATA_BY_IDENTIFIER]) + \
  p16(uds.DATA_IDENTIFIER_TYPE.VEHICLE_MANUFACTURER_SPARE_PART_NUMBER)
UDS_VM_SPARE_PART_RESPONSE = bytes([uds.SERVICE_TYPE.READ_DATA_BY_IDENTIFIER + 0x40]) + \
  p16(uds.DATA_IDENTIFIER_TYPE.VEHICLE_MANUFACTURER_SPARE_PART_NUMBER)
...
REQUESTS = [
  ...
  # Volkswagen
  (
    [EXTENDED_DIAGNOSTIC_REQUEST, UDS_VM_SPARE_PART_REQUEST],
    [EXTENDED_DIAGNOSTIC_RESPONSE, UDS_VM_SPARE_PART_RESPONSE]
  ),
]
...
def match_fw_to_car(fw_versions):
  ...
  for candidate, fws in candidates.items():
    for ecu, expected_versions in fws.items():
      ...
      ESSENTIAL_ECUS = [Ecu.engine, Ecu.eps, Ecu.esp, Ecu.fwdRadar, Ecu.fwdCamera, Ecu.vsa, Ecu.electricBrakeBooster. Ecu.gateway]
