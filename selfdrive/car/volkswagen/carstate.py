...
from selfdrive.car.volkswagen.values import DBC, CANBUS, NETWORK_MODEL, BUTTON_STATES, CarControllerParams
...
class CarState(CarStateBase):
  ...
  def update_mqb(self, pt_cp, cam_cp):
  ...
  def update_pq(self, pt_cp, cam_cp):
    ...
    # Update gear and/or clutch position data.
    can_gear_shifter = int(pt_cp.vl["Getriebe_1"]['Waehlhebelposition__Getriebe_1_'])
    ret.gearShifter = self.parse_gear_shifter(self.shifter_values.get(can_gear_shifter, None))

    # Update door and trunk/hatch lid open status.
    ret.doorOpen = bool(pt_cp.vl["Gateway_Komfort_1"]['Fahrertuerkontakt'])
    ...
    # preferences, including separate units for for distance vs. speed.
    # We use the speed preference for OP.
    ret.displayMetricUnits = not pt_cp.vl["Einheiten_1"]["MFA_v_Einheit_02"]
    ...
    ret.cruiseState.speed = cam_cp.vl["ACC_GRA_Anziege"]['ACA_V_Wunsch'] * CV.KPH_TO_MS
    if ret.cruiseState.speed > 70:
      ret.cruiseState.speed = 0

    # Update control button states for turn signals and ACC controls.
    ret.buttonStates["accelCruise"] = bool(pt_cp.vl["GRA_neu"]['Kurz_Tip_up']) or bool(pt_cp.vl["GRA_neu"]['Lang_Tip_up'])
    ret.buttonStates["decelCruise"] = bool(pt_cp.vl["GRA_neu"]['Kurz_Tip_down']) or bool(pt_cp.vl["GRA_neu"]['Lang_Tip_down'])
    ret.buttonStates["cancel"] = bool(pt_cp.vl["GRA_neu"]['Abbrechen'])
    ret.buttonStates["setCruise"] = bool(pt_cp.vl["GRA_neu"]['Setzen'])
    ret.buttonStates["resumeCruise"] = bool(pt_cp.vl["GRA_neu"]['Wiederaufnahme'])
    ret.buttonStates["gapAdjustCruise"] = bool(pt_cp.vl["GRA_neu"]['Zeitlueckenverstellung'])
    ret.leftBlinker = bool(pt_cp.vl["Kombi_1"]['Blinker_links_4_1'])
    ret.rightBlinker = bool(pt_cp.vl["Kombi_1"]['Blinker_rechts_4_1'])
    ...
    ret.espDisabled = pt_cp.vl["Bremse_1"]['ESP_Passiv_getastet']


  @staticmethod
  def get_can_parser(CP):
    if CP.networkModel == NETWORK_MODEL.MQB:
      signals = [
    ...
    elif CP.networkModel == NETWORK_MODEL.PQ:
      # this function generates lists for signal, messages and initial values
      signals = [
        # sig_name, sig_address, default
        ("LH3_LM", "Lenkhilfe_3", 0),                 # Absolute steering angle
        ("LH3_LMSign", "Lenkhilfe_3", 0),             # Steering angle sign
        ...
        # FIXME: Passenger and rear door states need research
        ...
        ("LH3_BLW", "Lenkhilfe_3", 0),                # Absolute driver torque input
        ("LH3_BLWSign", "Lenkhilfe_3", 0),            # Driver torque input sign
        ("LH2_Sta_HCA", "Lenkhilfe_2", 0),            # Steering rack HCA support configured
  ...
  def get_cam_can_parser(CP):
    if CP.networkModel == NETWORK_MODEL.MQB:
      ...
      signals = [
        ("ACA_V_Wunsch", "ACC_GRA_Anziege", 0),  # ACC cruise set point from J428 ACC radar
      ]

      checks = [
        ("ACC_GRA_Anziege", 50),  # From J428 ACC radar control module
      ]
