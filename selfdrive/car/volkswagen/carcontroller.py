...
class CarController():
  ...
  def update(self, enabled, CS, frame, actuators, visual_alert, audible_alert, leftLaneVisible, rightLaneVisible):
      ...
      if not enabled or (CS.standstill or CS.steeringFault):
        ...
        if apply_steer == 0:
          ...
          if self.hca_enabled_cnt >=  118 * 100:  # 118s ~ about 2 minutes
            # The Kansas Crosswind Problem: what happens if we do need to steer
    ...
    # car's message counter to change, and then send 16 of our own on the bus
    # exactly one counter increment ahead of the car's next 16 messages.
